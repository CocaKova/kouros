package com.cocakova.pygmalion.data
...
abstract class PygDb : RoomDatabase() {
    abstract fun servers(): ServerDao
    abstract fun workflows(): WorkflowDao
    abstract fun runs(): RunDao

    companion object {
        fun open(context: Context): PygDb =
            Room.databaseBuilder(context, PygDb::class.java, "pygmalion.db").build()
    }
}
