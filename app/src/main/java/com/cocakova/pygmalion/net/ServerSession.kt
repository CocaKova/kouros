...
import com.cocakova.pygmalion.core.api.ServerCapabilities
import com.cocakova.pygmalion.core.api.ServerEndpoint
import com.cocakova.pygmalion.core.api.SocketMessage
import com.cocakova.pygmalion.core.api.SystemStats
...
class ServerSession(
    ...
    private val _previews = MutableSharedFlow<com.cocakova.pygmalion.core.ws.BinaryFrame.Preview>(extraBufferCapacity = 4)
    /** Live preview frames; a slow collector simply misses frames (only the newest matters). */
    val previews: SharedFlow<com.cocakova.pygmalion.core.ws.BinaryFrame.Preview> = _previews.asSharedFlow()
            ...
            runCatching {
                client.socket(server.clientId).collect { m ->
                    when (m) {
                        ...
                        is SocketMessage.Binary -> (m.frame as? com.cocakova.pygmalion.core.ws.BinaryFrame.Preview)?.let { _previews.tryEmit(it) }
                    }
                }
            }.onFailure { e ->
                if (!opened) {
