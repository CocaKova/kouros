package com.cocakova.kouros.compile
            ...
            override fun onPageFinished(view: WebView, url: String) {
                ...
                val script = """
                    (function(){
                      ...
                      const tick = async () => {
                        const a = window.app;
                        if (!a || !a.graph || !a.graphToPrompt) { if (++tries > 240) { KourosOracle.fail('no app'); return; } setTimeout(tick, 250); return; }
                        try {
                          await a.loadGraphData(wf, true, true, 'kouros');
                          const r = await a.graphToPrompt();
                          KourosOracle.done(JSON.stringify(r.output));
                        } catch (e) { KourosOracle.fail(String(e)); }
                      };
                      tick();
                    })();
                """.trimIndent()
